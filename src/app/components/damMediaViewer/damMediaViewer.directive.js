    ...
    function damMediaViewerController($scope, $sce, $element) {
        ...
        vm.setMedia = function (val, dir) {
            ...
            /*else if(vm.media.type==="VIDEO" || vm.media.type==="AUDIO"){
                var source = {
                    src: $sce.trustAsResourceUrl(vm.media.url), type:"video/mp4"
                };
                vm.videogular.config.sources.push(source);
            }*/
            else if(vm.media.type==="VIDEO"){
                vm.videoUrl = $sce.trustAsResourceUrl(vm.media.urlStream);
            }
            else if(vm.media.type==="AUDIO"){
                vm.audioUrl = $sce.trustAsResourceUrl(vm.media.urlStream);
            }
            else {
                vm.imgUrl = vm.media.urlPreviewBig || vm.media.urlPreview || vm.media.url;
